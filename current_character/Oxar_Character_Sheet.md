# Øxar - Level 5 Battle Master

---

## COMBAT

| AC | HP | Speed | Init | Prof |
|:--:|:--:|:-----:|:----:|:----:|
| 16 | 59 | 30 ft | +1 | +3 |

| Saves | STR +7 | CON +6 |
|-------|:------:|:------:|

**Current HP:** _____ / 59

**Death Saves:** Success ○ ○ ○ | Failure ○ ○ ○

---

## ATTACKS

| Weapon | Hit | Damage | Notes |
|--------|:---:|--------|-------|
| **Greataxe** | +7 | 1d12+4 | 1-2→3, Cleave, +3 dmg w/ HWM (every Attack-action hit) |
| **Handaxe** | +7 | 1d6+4 | Thrown 20/60, Vex |

**Cleave:** On hit → free attack vs. adjacent creature (1d12 only, no mod)
**Vex:** On hit → Advantage on next attack vs. same target
**Heavy Weapon Mastery (GWM):** +3 damage on *every* hit with a Heavy weapon as part of the Attack action (not Hew or reactions; Cleave hit = DM call, RAW ambiguous)
**Savage Attacker:** 1/turn, roll damage dice twice, take either

---

## RESOURCES

| Resource | Track | Reset |
|----------|:-----:|-------|
| Hit Dice (1d10+3 heal) | ○ ○ ○ ○ ○ | Long: all |
| Second Wind (1d10+5 heal) | ○ ○ ○ | Short: 1, Long: all |
| Action Surge (+1 Action) | ○ | Short/Long |
| Superiority Dice (d8) (for maneuvers) | ○ ○ ○ ○ | Short/Long |
| Heroic Inspiration (re-roll any die, keep new roll) | ○ | Long |
| Rune Stone — Shield effect (Reaction, +5 AC) (my option to use as reaction) | ○ | Short |
| Rune Stone — Earth's Grasp (next weapon hit) (until I hit) (DC 13 STR save) | ○ | Long |

---

## TURN OPTIONS

### Action
- **Attack** → **2 attacks** (Extra Attack); 4 with Action Surge
- **Action Surge** → Take another Action this turn

### Bonus Action
- **Second Wind** → Heal 1d10+5 **+ move 15 ft, no Opportunity Attacks** (Tactical Shift)
- **Tactical Mind** → Failed ability check? Spend Second Wind, add 1d10 (if still fails, not expended)
- **Hew (GWM)** → After crit or kill with Heavy weapon, attack again with that weapon
- **Lunging Attack (1 die)** → Dash; move ≥5 ft straight before a melee hit (Attack action) → +1d8 dmg

### Reaction
- **Riposte** → Enemy misses you? Attack back +1d8 damage
- **Opportunity Attack** → Enemy leaves your reach
- **Rune Stone (Shield effect)** → +5 AC until start of next turn (vs. attack or Magic Missile)

### Free (1/turn each)
- **Cleave** → After Greataxe hit (one more attack 2nd creature, only weapon dmg)
- **Savage Attacker** → Roll damage dice twice, take either *(PHB p. 201)*

*(Heavy Weapon Mastery is not 1/turn — it's every Attack-action hit, see Attacks)*

---

## MANEUVERS (DC 15, cost 1 Superiority Die, max 1 maneuver per attack) — *PHB pp. 93–94*

**Superiority Dice:** ○ ○ ○ ○ *(d8, regain all on Short/Long Rest)*

| Maneuver | Trigger | Effect |
|----------|---------|--------|
| **Goading** | On hit | +1d8 dmg, target has Disadv attacking others (WIS neg) |
| **Lunging** | Bonus Action | Dash; move ≥5 ft straight before melee hit (Attack action) → +1d8 dmg |
| **Riposte** | Enemy misses | Reaction attack, +1d8 dmg |

---

## SKILLS

| Skill | Mod | | Skill | Mod |
|-------|:---:|-|-------|:---:|
| Athletics | +7 | | Stealth | +4 (Disadv) |
| Intimidation | +2 | | Survival | +3 |
| Perception | +3 | | Animal Handling | +3 |

**Passive Perception:** 13

**Tools:** Gaming set, Brewer's Supplies

**Languages:** Common, *(2 open — every 2024 PC knows Common + two standard languages, PHB p. 37)*

---

# FIGHTING TECHNIQUES

---

## Standard Turn
- **Two greataxe swings** (+7, 1d12+7 each with HWM). Pick a target whose neighbor you also want dead — a hit lets **Cleave** carry into the creature next to it (free 1d12, once/turn)
- **Savage Attacker** on the biggest damage roll of the turn (1/turn)
- Spend a superiority die when it matters: **Goading** (DC 15 WIS) on the enemy you want locked onto you

## Opening Moves
- **Out of reach?** Throw a handaxe (+7, 20/60) — a hit triggers **Vex**: Advantage on your next attack vs. that target
- **Need to close fast?** **Lunging Attack** (Bonus Action, 1 die): Dash, come in ≥5 ft straight → +1d8 on a melee hit this turn

## Nova Turn *(the decisive round — boss revealed, ally down, breach to hold)*
1. **Prime Earth's Grasp beforehand** (1/long rest): your first hit forces DC 13 STR save → target **Restrained** — your remaining attacks *and the whole party's* get Advantage, its attacks have Disadvantage
2. Attack (2 swings) → **Action Surge** → Attack again = **4 × (1d12+7)**
3. Maneuver die on key hits (max 1 per attack)
4. Any kill or crit → **Hew** (Bonus Action): 5th swing

## Your One Reaction — spend it wisely
1. **Rune Stone Shield** *(1/short rest)* — a big attack is about to hit by ≤5? AC 16 → 21, likely turning it into a miss. Auto-negates *Magic Missile*
2. **Riposte** *(1 die)* — melee enemy missed you? Punish: +7, 1d12+4+1d8
3. **Opportunity Attack** — enemy leaves your reach

## Staying Alive
- **Second Wind** (Bonus Action, 3/rest): heal 1d10+5 **and** shift 15 ft with no Opportunity Attacks — heal-and-reposition, or escape a surround
- Playing tank: **Goad** the biggest hitter — Disadvantage on attacks against anyone but you, and you have AC 16 + the Shield stone
- **Heroic Inspiration**: hold it to re-roll a failed save or a just-missed key attack (you re-roll *after* seeing the result)

## Resource Pacing
- **Superiority dice (4)**: refresh on *short* rest — spend 1-2 per fight, don't hoard
- **Action Surge (1/rest)**: for the decisive round, not round one by habit
- **Rune Stone**: Shield freely (short rest); **Earth's Grasp only once per day** — save it for the fight that matters

---

# REFERENCE

---

## Character Info

| | |
|---|---|
| **Name** | Øxar |
| **Species** | Human *(PHB p. 194)* |
| **Class** | Fighter 5 (Battle Master) *(PHB pp. 90, 93)* |
| **Background** | Soldier *(PHB p. 185)* |
| **Alignment** | Chaotic Good |

---

## Ability Scores

| STR | DEX | CON | INT | WIS | CHA |
|:---:|:---:|:---:|:---:|:---:|:---:|
| 18 | 12 | 16 | 10 | 11 | 8 |
| +4 | +1 | +3 | +0 | +0 | -1 |

---

## Features

**Great Weapon Fighting** — Damage rolls of 1-2 become 3 (two-handed weapons) *(PHB p. 209)*

**Tough** — +2 HP per level (+10 at level 5) *(PHB p. 202)*

**Heroic Inspiration** — Gain after Long Rest; expend to re-roll any die just rolled, must use the new roll. If you gain it while you already have it, you may give it to a party member *(PHB p. 368)*

**Tactical Mind** — Spend Second Wind on failed ability check → +1d10 (refunded if still fails) *(PHB p. 91)*

**Extra Attack** *(L5)* — Attack twice when you take the Attack action *(PHB p. 92)*

**Tactical Shift** *(L5)* — When you use Second Wind as a Bonus Action, move up to half your Speed without provoking Opportunity Attacks *(PHB p. 92)*

**Great Weapon Master** *(L4 feat, PHB 2024 p. 204)*
- +1 STR (applied)
- **Heavy Weapon Mastery** — +PB damage on *each* hit with a Heavy weapon as part of the Attack action
- **Hew** — Bonus Action attack with Heavy weapon after crit or reducing creature to 0 HP

---

## Equipment

### Weapons *(PHB p. 215; mastery properties p. 214)*
- **Greataxe** — 1d12+4, Heavy, Two-Handed, Cleave mastery
- **Handaxes (2)** — 1d6+4, Light, Thrown 20/60, Vex mastery
- **Greatsword** *(mastery known, not owned)* — 2d6+4, Graze mastery
- **⚠️ Open mastery slot** — Fighter L4 grants a 4th Weapon Mastery *(not yet chosen; may also swap one mastery choice each Long Rest)*

### Armor
- **Chain Mail** — AC 16, Stealth Disadvantage *(PHB p. 219)*

### Gear (Dungeoneer's Pack)
Backpack, Crowbar, Hammer, Pitons (10), Rope (50 ft), Torches (10), Tinderbox, Rations (10 days), Waterskin

### Magic Items

**Rune Stone** — Single stone bearing one carved rune. The rune offers two possible effects; each effect has its own recharge.

- **Shield effect** *(Reaction; recharge: Short Rest)* — Casts the *Shield* spell *(PHB p. 316)*. +5 AC until the start of your next turn (applies to the triggering attack); negates *Magic Missile*.
- **Earth's Grasp effect** *(recharge: Long Rest)* — On your next hit with a weapon attack, target makes a **DC 13 STR save** or cannot move on its next turn and counts as **Restrained** *(PHB p. 373)*.

---

## HP Calculation

| Level | HP |
|-------|-----|
| 1 | 10 + 3 (CON) = 13 |
| 2 | 6 + 3 = 9 |
| 3 | 6 + 3 = 9 |
| 4 | 6 + 3 = 9 |
| 5 | 6 + 3 = 9 |
| Tough | +10 |
| **Total** | **59** |

---

## Planned Features

### Level 8: Sentinel *(PHB p. 207)*
- Opportunity Attack → target's speed becomes 0
- Disengage doesn't prevent your Opportunity Attacks
- Reaction attack when adjacent creature attacks someone else

---

*Last Updated: 2026-07-31*
